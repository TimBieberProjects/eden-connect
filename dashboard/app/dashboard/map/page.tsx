...
const STAGE_CONFIG = {
  kicked_off:  { label: 'Program Kicked Off',      color: '#2F6FB2', bg: 'bg-eden-pale-blue', text: 'text-eden-blue', border: 'border-eden-blue/30', dot: 'bg-eden-blue' },
    ...
    <div className="relative flex" style={{ height: 'calc(100vh - 56px)' }}>
      ...
      <div className="absolute top-4 left-[272px] z-10">
        <button
          ...
          className="bg-eden-teal hover:bg-eden-teal/90 text-navy text-sm font-bold px-4 py-2 rounded-lg shadow-lg transition flex items-center gap-2"
        >
        ...
        <div className="absolute top-0 right-0 h-full w-80 bg-white shadow-2xl border-l border-slate-200 z-20 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {selected.health_committee_contact && (
              <div className="flex items-center gap-2 bg-eden-pale-blue border border-eden-blue/20 rounded-xl p-3">
                <div className="w-8 h-8 rounded-full bg-eden-teal/15 flex items-center justify-center flex-shrink-0">
                  <svg className="w-4 h-4 text-navy" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                </div>
                <div>
                  <div className="text-xs text-eden-blue font-medium">Health Committee</div>
                  <div className="text-sm font-semibold text-navy">{selected.health_committee_contact}</div>
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-slate-100">
            <button
              ...
              className="w-full text-sm bg-navy hover:bg-navy/90 text-eden-white font-bold py-2.5 rounded-lg transition"
            >
              Edit this community
            </button>
          </div>
          ...
          <form onSubmit={handleAddCommunity} className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              ...
              <input required value={addForm.name} onChange={e => setAddForm(f => ({ ...f, name: e.target.value }))}
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-eden-teal outline-none" />
            </div>
            ...
            <div>
              ...
              <select required value={addForm.province} onChange={e => setAddForm(f => ({ ...f, province: e.target.value }))}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-eden-teal outline-none text-slate-700">
            ...
            <div>
              ...
              <textarea value={addForm.description} onChange={e => setAddForm(f => ({ ...f, description: e.target.value }))}
                ...
                rows={3}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-eden-teal outline-none resize-none" />
            </div>
            ...
            <button type="submit" disabled={saving}
              className="w-full bg-eden-teal hover:bg-eden-teal/90 disabled:bg-eden-teal/40 text-navy font-bold py-2.5 rounded-lg text-sm transition">
